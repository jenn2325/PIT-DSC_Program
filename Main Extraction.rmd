...
for (i in 1:59){
  ...
  d = data.frame(District = str_extract(all_files[i], "(bx|qn|mn|si|bk)[0-9]{1,4}") ,Helpful_neighbours = m)
  hn = rbind(hn, d)
}
...
Air_Pollution <- whoWeAre %>% select(District, Name) %>% left_join(., Air_Pollution, by = c("Name" = "Community.District"))
colnames(Air_Pollution) <- c("Boroughs", "Districts","District_long_name", "Air Pollution")
```
...
for (i in 1:59){
  ...
  d = data.frame(District = str_extract(all_files[i], "(bx|qn|mn|si|bk)[0-9]{1,4}") ,infant_mortality = m) %>% 
...
for (i in 1:59){
  ...
  d = data.frame(District = str_extract(all_files[i], "(bx|qn|mn|si|bk)[0-9]{1,4}") ,premature_deaths = m)
  pd = rbind(pd, d)
}
